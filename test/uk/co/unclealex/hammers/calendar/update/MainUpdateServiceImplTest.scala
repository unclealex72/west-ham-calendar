...
class MainUpdateServiceImplTest extends Specification with MockFactory {
  ...
  "Adding a completely new game" should {
    "create and update a new game" in {
      ...
      val expectedStoredGame = game(SOUTHAMPTON) { g =>
        g.dateTimePlayed = Some(September(5, 2013) at (15, 0))
      }
  ...
  "Updating an existing game with new ticket information" should {
    "update the game" in {
      ...
      val expectedStoredGame = game(SOUTHAMPTON) { g =>
        g.dateTimePlayed = Some(September(5, 2013) at (15, 0))
        g.dateTimeSeasonTicketsAvailable = Some(September(3, 2013) at (9,0))
      }
      (s.gameDao.store _) expects(where(expectedStoredGame)) returning(expectedStoredGame)
      s.mainUpdateService.processDatabaseUpdates()
    }
   ...
   */
  implicit val gameMatcher: Game => (Game => Boolean) = { g1 => g2 =>
    g1.gameKey == g2.gameKey &&
    g1.attended == g2.attended &&
    g1.attendence == g2.attendence &&
    g1.dateTimeAcademyMembersAvailable == g2.dateTimeAcademyMembersAvailable &&
    g1.dateTimeBondholdersAvailable == g2.dateTimeBondholdersAvailable &&
    g1.dateTimeGeneralSaleAvailable == g2.dateTimeGeneralSaleAvailable &&
    g1.dateTimePlayed == g2.dateTimePlayed &&
    g1.dateTimePriorityPointPostAvailable == g2.dateTimePriorityPointPostAvailable &&
    g1.dateTimeSeasonTicketsAvailable == g2.dateTimeSeasonTicketsAvailable &&
    g1.id == g2.id &&
   ...
   */
  def game: GameKey => (Game => Unit) => Game = { gameKey => f => 
