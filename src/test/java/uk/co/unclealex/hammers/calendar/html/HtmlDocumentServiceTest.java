...
public class HtmlDocumentServiceTest extends SpringTest {
	...
	public void testDocumentLoads() {
		try {
			getHtmlDocumentService().load(getTestResourceUrl());
		}
		catch (IOException e) {
			fail("Could not load the test resource");
		}
	}

	public HtmlDocumentService getHtmlDocumentService() {
		return autowire(new HtmlDocumentServiceImpl());
	}
}
